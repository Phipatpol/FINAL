...
app.get('/delete',(re1, res) => {
    pool.getConnection((err, connection) =>{
        ...
        connection.query('SELECT * FROM lotery_16_ma ', (err, rows) => {
            connection.release();
            if(!err){ 
                //--------Model of Data--------------//
...
app.get('/:id',(req, res) => {
    ...
    pool.getConnection((err, connection) =>{ //err คือ connect ไม่ได้ or connection คือ connect ได้บรรทัดที่ 13-19
        ...
        connection.query('SELECT * FROM lotery_16_ma  WHERE `id` = ?', req.params.id, (err, rows) => {
            connection.release();
            if(!err){ 
                //res.send(rows)
                obj = {beers : rows, Error, err}
...
app.get('/getname_id/:name&:id',(req, res) => {
    ...
    pool.getConnection((err, connection) =>{ //err คือ connect ไม่ได้ or connection คือ connect ได้บรรทัดที่ 13-19
        ...
        //แก้ไขคำสั่ง SQL
        connection.query('SELECT * FROM lotery_16_ma  WHERE `id` = ? OR `name` LIKE ?', [req.params.id, req.params.name], (err, rows) => {
            connection.release();
            if(!err){ 
                obj = {beers : rows, Error, err}
        ...
        if(err) throw err
            const params = req.body
                ...
                pool.getConnection((err, connection2) => {
                    connection2.query(`SELECT COUNT(id) AS count FROM lotery_16_ma WHERE id = ${params.id}`, (err, rows) => {
                        if(!rows[0].count){
                            connection.query('INSERT INTO lotery_16_ma SET ?', params, (err, rows) => {
                                connection.release()
                                if(!err){
...
app.post('/delete',(req, res) => {     //ใช้ POST เข้าจัดการจะปลอดภัยกว่าแบบเดิม
    ...
    pool.getConnection((err, connection) =>{
        ...
        connection.query('DELETE FROM `lotery_16_ma` WHERE `beers`.`id` = ?', [id], (err, rows) => {
            connection.release();
            if(!err){ 
...
app.put('/update',(req, res) => {
    pool.getConnection((err, connection) =>{
        ...
        connection.query('UPDATE lotery_16_ma  SET Category = ?, Draw = ?, Date = ?, image = ? WHERE id = ?', [name, tagline, description, image, id], (err, rows) => {
            connection.release();
            if(!err){ 
